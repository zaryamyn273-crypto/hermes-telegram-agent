...
def parse_summary_request(text: str) -> Tuple[bool, int]:
    ...
    patterns = [
        ...
        r"(\d+)\s*(?:تا\s*)?(?:پیام|چت|گفتگو|مسیج)\s*(?:اخیر|قبل|قبلی|گذشته)?\s*(?:رو|را)?\s*(?:خلاصه|گزارش)\s*(?:کن|بکن|بده|بگو)",
        ...
        r"(\d+)\s*(?:تا\s*)?(?:پیام|چت|مسیج|گفتگو)\s*.*(?:قبل|قبلی|اخیر|گذشته|پیش|توضیح|بررسی|بخون|خلاصه)",

        # Inquiries about what happened / was discussed in past messages
